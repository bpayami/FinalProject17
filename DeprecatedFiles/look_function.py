...
class TextAdventureCmd(cmd.Cmd):
    ...
    def do_look(self, item):
        ...
        """
        #COMPLETELY WORKING VERSION BEFORE CONSOLIDATION
        for item in worldItems:
            if choice in item.names:
                if current.official in worldRooms[location]['GROUND']:
                    print(current.official)
                    print('its in the room')
                    if current.validity == True:
                        print('maybe im on to something')
                if current.official not in worldRooms[location]['GROUND']:
                    print('its not in the room')
                    if current.validity == False:
                        print('You do not see that item.') #perhaps can be ommitted
        """

        """
        #DELETED SNIPPET OF LOOK FUNCTION
        for item in worldItems:
            if current not in item.names:
                validity = 'no x1'
            else:
                if current.official not in worldRooms[location]['GROUND']:
                    validity = 'no x2'
