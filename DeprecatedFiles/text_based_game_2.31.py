...
lockedroom = Room(location, worldRooms[location]['DESC'], back=False, left=False, right=False)
study = Room(location, worldRooms[location]['DESC'], left=False, right=False)
library = Room(location, worldRooms[location]['DESC'], left=False, right=False)
#has special case of 'up' ^^^^^^^
secretattic = Room(location, worldRooms[location]['DESC'], front=False, back=False, left=False, right=False, down=True) #ADD DOWN=FALSE TO CLASS
#special case ^^^^^^^
...
#alt maybe:  for room in worldRooms[location]:

print(location)

if True == True:
    if location in worldWings.keys():
        if location == 'Wing A':
            wingA.altnextTurn()
            user_input = str(input('Where would you like to travel? ')).upper()
...
#write out the class - room instructions for each room
#create a continious for loop
